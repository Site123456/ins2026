...
export default function FooterDef({ isDark = true, accent: accentProp }: FooterProps) {
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.isSubscribed) {
        // User exists, ask them to login first
        openAuthModal('signin', email);
      } else {
        // New user, open newsletter flow (which asks for name as part of AuthModal if needed)
  ...
  const FacebookIcon = ({ isDark, ...props }: SVGProps<SVGSVGElement> & { isDark?: boolean }) => (
  ...
  const TwitterIcon = ({ isDark, ...props }: SVGProps<SVGSVGElement> & { isDark?: boolean }) => (
    <svg viewBox="0 0 24 24" fill="currentColor" {...props}>
  ...
  const InstagramIcon = ({ isDark, ...props }: SVGProps<SVGSVGElement> & { isDark?: boolean }) => (
    <svg viewBox="0 0 24 24" fill="currentColor" {...props}>
  ...
  const YoutubeIcon = ({ isDark, ...props }: SVGProps<SVGSVGElement> & { isDark?: boolean }) => (
    <svg viewBox="0 0 24 24" fill="currentColor" {...props}>
  ...
  const socials = [
    { icon: FacebookIcon, href: "https://www.facebook.com/people/Indian-Nepali-Swad/100041638634787/", label: "Facebook" },
    { icon: TwitterIcon, href: "https://twitter.com/INepaliswad", label: "Twitter" },
    { icon: InstagramIcon, href: "https://www.instagram.com/indiannepaliswad/", label: "Instagram" },
    { icon: YoutubeIcon, href: "https://www.youtube.com/channel/UCHPsdHfepFygMiWvlLclhcA", label: "YouTube" },
    { icon: TikTokIcon, href: "https://www.tiktok.com/@indiannepaliswad", label: "TikTok" },
  ];

  return (
    <footer className={`relative w-full overflow-hidden ${isDark ? "bg-[#08080c] text-white" : "bg-[#f5f4f2] text-gray-900"}`}>
      <div className="h-px w-full" style={{ background: `linear-gradient(to right, transparent, ${hexToRgba(accent, 0.4)}, transparent)` }} />
      ...
      <div className="relative mx-auto max-w-6xl px-5 py-10 md:px-8 md:py-14">
        <div className="grid gap-10 md:grid-cols-12 md:gap-12">
          ...
          <div className="md:col-span-12 lg:col-span-4 flex flex-col items-center text-center lg:items-start lg:text-left space-y-7">
            <div className={`text-[11px] font-semibold uppercase tracking-wider ${isDark ? "text-white/30" : "text-black/30"}`}>
              {language === 'en' ? "Subscribe to Newsletter" : "Souscrire à la Newsletter"}
            </div>
            <form onSubmit={handleSubscribe} className="w-full max-w-sm mx-auto lg:mx-0">
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 rounded-2xl p-1 transition-all duration-300"
                style={{
                  borderWidth: 1, borderColor: focused ? hexToRgba(accent, 0.4) : isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.06)",
                  backgroundColor: focused ? hexToRgba(accent, 0.05) : isDark ? "rgba(255,255,255,0.02)" : "rgba(0,0,0,0.02)",
                  boxShadow: focused ? `0 4px 20px -4px ${hexToRgba(accent, 0.15)}` : "none",
                }}>
                <input
                  ...
                  disabled={isLoading}
                  className={`flex-1 bg-transparent py-2.5 px-4 text-[13px] outline-none w-full ${isDark ? "text-white" : "text-black"}`}
                />
                <motion.button type="submit" disabled={isLoading} className="flex items-center justify-center gap-1.5 rounded-xl px-5 py-2.5 text-[12px] font-bold transition-all duration-300 text-white disabled:opacity-50"
                  style={{ backgroundColor: accent }} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                  {isLoading ? "..." : language === "en" ? "Subscribe" : "S'abonner"}
                  <ArrowRight className="h-3.5 w-3.5" />
                </motion.button>
              </div>
            </form>

            <p className={`text-[11px] leading-relaxed max-w-xs mx-auto lg:mx-0 ${isDark ? "text-white/50" : "text-black/50"}`}>
              {language === 'en' ? "By subscribing, you'll receive our exclusive offers and menu updates right to your inbox." : "En vous abonnant, vous recevrez nos offres exclusives et les nouveautés de notre menu directement par email."}
            </p>

            <div className={`text-[11px] font-semibold uppercase tracking-wider ${isDark ? "text-white/30" : "text-black/30"}`}>
              {language === 'en' ? "Table Reservation" : "Réservation de table"}
            </div>
            <div className="flex flex-col w-full max-w-sm space-y-3">
              {[
                { label: language === 'en' ? "Book at Aubervilliers" : "Réserver à Aubervilliers", href: "https://reservation.indian-nepaliswad.fr/aubervilliers" },
                { label: language === 'en' ? "Book at Paris 15" : "Réserver à Paris 15", href: "https://reservation.indian-nepaliswad.fr/paris15" },
              ].map((btn) => (
                <a key={btn.label} href={btn.href} className="group w-full py-3.5 rounded-xl text-[14px] font-semibold flex items-center justify-center gap-2 transition-all duration-300"
                  style={{ backgroundColor: accent, color: "white", boxShadow: `0 4px 14px -4px ${hexToRgba(accent, 0.45)}` }}
                  onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = hexToRgba(accent, 0.85); e.currentTarget.style.boxShadow = `0 6px 22px -4px ${hexToRgba(accent, 0.55)}`; }}
                  onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = accent; e.currentTarget.style.boxShadow = `0 4px 14px -4px ${hexToRgba(accent, 0.45)}`; }}>
                  <svg className="h-4 w-4 opacity-90 group-hover:opacity-100 transition-all font-bold" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                    <path d="M17.657 16.657L13 21.314l-4.657-4.657A8 8 0 1117.657 16.657z" /><circle cx="12" cy="12" r="3" />
                  </svg>
                  {btn.label}
                </a>
              ))}
            ...
            <div className="flex flex-wrap justify-center lg:justify-start items-center gap-2 pt-1">
              {socials.map(({ icon: Icon, href, label }) => (
                <a key={label} href={href} aria-label={label} target="_blank" rel="noopener noreferrer"
                  ...
                  style={{ backgroundColor: isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.04)", color: isDark ? "rgba(255,255,255,0.4)" : "rgba(0,0,0,0.4)" }}
                  onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = hexToRgba(accent, 0.12); e.currentTarget.style.color = accent; }}
                  onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.04)"; e.currentTarget.style.color = isDark ? "rgba(255,255,255,0.4)" : "rgba(0,0,0,0.4)"; }}>
      ...
      <div className="px-6 pb-6 max-w-6xl mx-auto text-start">
        <div className="space-y-2 text-[10px] sm:text-[11px] leading-relaxed opacity-40">
          ...
          <p>
            {language === 'en'
              ? "Your privacy and security are our highest priority. All exchanges are secured via advanced encryption (TLS 1.3) and no passwords are ever stored, as access relies purely on secure OTP verification."
              : "L’accès à votre compte peut s’effectuer sans mot de passe grâce à un système sécurisé d’authentification par code OTP. Vous pouvez vous connecter soit via un code reçu par email. Aucun mot de passe n’est stocké, ce qui réduit fortement les risques d’usurpation."}
          </p>
          <p>
            {language === 'en'
              ? "For more information, please consult our privacy policy available on our website."
              : "Pour plus d’informations, veuillez consulter notre politique de confidentialité disponible sur notre site web."}
          </p>
        </div>
      </div>

      <div className="border-t" style={{ borderColor: isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.04)" }}>
        <div className="mx-auto max-w-6xl px-5 py-6 md:px-8">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className={`flex items-center justify-center gap-2 text-[11px] ${isDark ? "text-white/20" : "text-black/20"}`}>
              <span className="font-bold tracking-tight" style={{ color: hexToRgba(accent, isDark ? 0.4 : 0.5) }}>
                INDIAN NEPALI SWAD
              </span>
              <span>&copy;2017-{year}</span>
              <span className="hidden sm:inline">•</span>
              <span className="hidden sm:inline">{language === 'en' ? "All rights reserved" : "Tout droit reservé"}</span>
            </div>
          </div>
        </div>
      </div>
